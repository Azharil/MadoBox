<?php
	...
	function copy_avatar($source, $fn)
	{
		...
		if( $C->IMAGE_MANIPULATION == "imagemagick_cli" ) {
			...
			if( $tp==IMAGETYPE_GIF && !file_exists($fn0) ) {
				...
				if( file_exists($tmp0) ) {
					...
					system( 'rm '.$C->IMG_DIR.'avatars/full/'.$tmp.'*' );
				}
			}
		}
		else {
			...
			$dstp4	= imagecreatetruecolor(1028, 706);
			...
			$res4	= imagecopyresampled($dstp4, $srcp, 0, 0, 0, 0, 1028, 706, min($w,$h), min($w,$h));
			imagedestroy($srcp);
		...
		if( !file_exists($fn0) || !file_exists($fn1) || !file_exists($fn2) || !file_exists($fn3) || !file_exists($fn4) ) {
			rm($fn0, $fn1, $fn2, $fn3);
			return FALSE;
		}
		chmod( $fn0, 0777 );
