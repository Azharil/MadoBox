	...
	class network
	{
		...
		public function get_mostfollowed_groups($force_refresh=FALSE)
		{
			...
			return $data;
				...
				public function get_likes_anime($force_refresh=FALSE)
		{
			if( ! $this->id ) {
				return FALSE;
			}
			...
			$data	= array();
			$this->db2->query('SELECT id, groupname, avatar, num_followers AS c FROM `groups` WHERE num_followers>0 ORDER BY num_followers DESC LIMIT 3');
			while($obj = $this->db2->fetch_object()) {
				if( empty($obj->avatar) ){
					$obj->avatar = $GLOBALS['C']->DEF_AVATAR_GROUP;
				}
