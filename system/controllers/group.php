<?php
	...
	$tabs	= array('episodes', 'members', 'trailer', 'addepisode');
	...
	$subtabs	= array();
	if( $D->tab == 'settings' ) {
		$subtabs = array('main', 'cover', 'admins', 'rssfeeds', 'delgroup');
		if( $g->is_private ) {
			$subtabs[]	= 'privmembers';
		}
		$D->subtab	= 'main';
	...
	$D->atype	= nl2br(htmlspecialchars($D->g->atype));
	$D->aaired	= nl2br(htmlspecialchars($D->g->aaired));
	$D->aeps	= nl2br(htmlspecialchars($D->g->aeps));
	$D->agenre	= nl2br(htmlspecialchars($D->g->agenre));
	$D->aduration	= nl2br(htmlspecialchars($D->g->aduration));
	$D->arating	= nl2br(htmlspecialchars($D->g->arating));
	$D->atrailer	= nl2br(htmlspecialchars($D->g->atrailer));



	

			@$D->ifaction	= nl2br(htmlspecialchars($D->g->ifaction));
		@$D->ifcomedy	= nl2br(htmlspecialchars($D->g->ifcomedy));
		@$D->ifdrama	= nl2br(htmlspecialchars($D->g->ifdrama));
		@$D->ifecchi	= nl2br(htmlspecialchars($D->g->ifecchi));
		@$D->iffantasy	= nl2br(htmlspecialchars($D->g->iffantasy));
		@$D->ifgame	= nl2br(htmlspecialchars($D->g->ifgame));
		@$D->ifhorror	= nl2br(htmlspecialchars($D->g->ifhorror));
		@$D->ifmagic	= nl2br(htmlspecialchars($D->g->ifmagic));
		@$D->ifmystery	= nl2br(htmlspecialchars($D->g->ifmystery));
		@$D->ifromance	= nl2br(htmlspecialchars($D->g->ifromance));
		@$D->ifschool	= nl2br(htmlspecialchars($D->g->ifschool));
		@$D->ifscifi	= nl2br(htmlspecialchars($D->g->ifscifi));
		@$D->ifshoujo	= nl2br(htmlspecialchars($D->g->ifshoujo));
		@$D->ifshonen	= nl2br(htmlspecialchars($D->g->ifshonen));
		@$D->ifsupernatural	= nl2br(htmlspecialchars($D->g->ifsupernatural));
		@$D->ifsuperpower	= nl2br(htmlspecialchars($D->g->ifsuperpower));
		@$D->ifsliceoflife	= nl2br(htmlspecialchars($D->g->ifsliceoflife));
		@$D->ifsports	= nl2br(htmlspecialchars($D->g->ifsports));
	...
	elseif( $D->tab == 'settings' && $D->i_am_admin )
	{
		...
		if( $D->subtab == 'main' ) {
			...
			$D->aaired	= $g->aaired;
			$D->agenre	= $g->agenre;			
			$D->aduration	= $g->aduration;
			$D->arating	= $g->arating;
			$D->atrailer	= $g->atrailer;



			//genre
			
	@$D->ifaction	= $g->ifaction;
			@$D->ifcomedy	= $g->ifcomedy;
			@$D->ifecchi	= $g->ifecchi;
			@$D->ifgame	= $g->ifgame;
			@$D->ifhorror	= $g->ifhorror;
			@$D->ifmystery	= $g->ifmystery;
			@$D->ifmagic	= $g->ifmagic;
			@$D->ifromance	= $g->ifromance;
			@$D->ifschool	= $g->ifschool;
			@$D->ifscifi	= $g->ifscifi;
			@$D->ifshoujo	= $g->ifshoujo;
			@$D->ifshonen	= $g->ifshonen;
			@$D->ifsupernatural	= $g->ifsupernatural;
			@$D->ifsuperpower	= $g->ifsuperpower;
			@$D->ifdrama	= $g->ifdrama;
			@$D->ifsliceoflife	= $g->ifsliceoflife;
			@$D->ifsports	= $g->ifsports;
			...
			if( isset($_POST['sbm']) ) {
				...
				$D->atype		= trim($_POST['atype']);
				$D->aeps		= trim($_POST['aeps']);
				
				$D->aaired		= trim($_POST['aaired']);
				$D->agenre		= trim($_POST['agenre']);
				$D->aduration	= trim($_POST['aduration']);
				$D->arating		= trim($_POST['arating']);
				$D->atrailer		= trim($_POST['atrailer']);
				...
				@$D->ifaction	= trim($_POST['ifaction']);
			@$D->ifcomedy	= trim($_POST['ifcomedy']);
			@$D->ifecchi	= trim($_POST['ifecchi']);
			@$D->ifgame	= trim($_POST['ifgame']);
			@$D->ifhorror	= trim($_POST['ifhorror']);
			@$D->ifmystery	= trim($_POST['ifmystery']);
			@$D->ifmagic	= trim($_POST['ifmagic']);
			@$D->ifromance	= trim($_POST['ifromance']);
			@$D->ifschool	= trim($_POST['ifschool']);
			@$D->ifscifi	= trim($_POST['ifscifi']);
			@$D->ifshoujo	= trim($_POST['ifshoujo']);
			@$D->ifshonen	= trim($_POST['ifshonen']);
			@$D->ifsupernatural	= trim($_POST['ifsupernatural']);
			@$D->ifsuperpower	= trim($_POST['ifsuperpower']);
			@$D->ifdrama	= trim($_POST['ifdrama']);
			@$D->ifsliceoflife	= trim($_POST['ifsliceoflife']);
			@$D->ifcomedy	= trim($_POST['ifcomedy']);
			@$D->iffantasy	= trim($_POST['iffantasy']);
			@$D->ifsports	= trim($_POST['ifsports']);
				...
				if( isset($_FILES['form_avatar']) && is_uploaded_file($_FILES['form_avatar']['tmp_name']) ) {
					...
					$D->atype	= mb_substr($D->atype, 0, $C->POST_MAX_SYMBOLS);
				$db2->query('UPDATE groups SET atype="'.$db2->e($D->atype).'" WHERE id="'.$g->id.'" LIMIT 1');
				$D->aeps	= mb_substr($D->aeps, 0, $C->POST_MAX_SYMBOLS);
				$db2->query('UPDATE groups SET aeps="'.$db2->e($D->aeps).'" WHERE id="'.$g->id.'" LIMIT 1');
				$D->aaired	= mb_substr($D->aaired, 0, $C->POST_MAX_SYMBOLS);
				$db2->query('UPDATE groups SET aaired="'.$db2->e($D->aaired).'" WHERE id="'.$g->id.'" LIMIT 1');
				$D->agenre	= mb_substr($D->agenre, 0, $C->POST_MAX_SYMBOLS);
				$db2->query('UPDATE groups SET agenre="'.$db2->e($D->agenre).'" WHERE id="'.$g->id.'" LIMIT 1');
				$D->aduration	= mb_substr($D->aduration, 0, $C->POST_MAX_SYMBOLS);
				$db2->query('UPDATE groups SET aduration="'.$db2->e($D->aduration).'" WHERE id="'.$g->id.'" LIMIT 1');
				$D->arating	= mb_substr($D->arating, 0, $C->POST_MAX_SYMBOLS);
				$db2->query('UPDATE groups SET arating="'.$db2->e($D->arating).'" WHERE id="'.$g->id.'" LIMIT 1');
                $db2->query('UPDATE groups SET atrailer="'.$db2->e($D->atrailer).'" WHERE id="'.$g->id.'" LIMIT 1');
				...
				//genre
				$D->ifaction	= mb_substr($D->ifaction, 0, $C->POST_MAX_SYMBOLS);
				$db2->query('UPDATE groups SET if_action="'.$db2->e($D->ifaction).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_drama="'.$db2->e($D->ifdrama).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_comedy="'.$db2->e($D->ifcomedy).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_ecchi="'.$db2->e($D->ifecchi).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_fantasy="'.$db2->e($D->iffantasy).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_game="'.$db2->e($D->ifgame).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_horror="'.$db2->e($D->ifhorror).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_magic="'.$db2->e($D->ifmagic).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_mystery="'.$db2->e($D->ifmystery).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_romance="'.$db2->e($D->ifromance).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_school="'.$db2->e($D->ifschool).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_scifi="'.$db2->e($D->ifscifi).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_shoujo="'.$db2->e($D->ifshoujo).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_shonen="'.$db2->e($D->ifshonen).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_sliceoflife="'.$db2->e($D->ifsliceoflife).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_supernatural="'.$db2->e($D->ifsupernatural).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_superpower="'.$db2->e($D->ifsuperpower).'" WHERE id="'.$g->id.'" LIMIT 1');
				$db2->query('UPDATE groups SET if_sports="'.$db2->e($D->ifsports).'" WHERE id="'.$g->id.'" LIMIT 1');
	...
	$D->page_title	= "Cover";
			...
			$D->errmsg	= '';
			    ...
			    $dirname = $C->IMG_DIR ."cover/default/";
	if( isset($_POST['sbm']) ) {
	
	
	if( isset($_FILES['form_cover']) && is_uploaded_file($_FILES['form_cover']['tmp_name']) ) {
					$f	= (object) $_FILES['form_cover'];
					...
					elseif( $w<$C->AVATAR_SIZE || $h<$C->AVATAR_SIZE ) {
						...
						if( ! $res) {
							...
							$old	= $g->avatar;
							if( $old != $C->DEF_AVATAR_GROUP ) {
								rm( $C->IMG_DIR.'avatar/full/'.$old );
								
							}
							$db2->query('UPDATE groups SET cover_anime="'.$db2->escape($fn).'" WHERE id="'.$g->id.'" LIMIT 1');
	...
	$D->cover	= $db2->fetch_field('SELECT cover_anime FROM groups WHERE id='.$g->id);
	
	function titid() {
	$homo = 1; 
	while ($homo <=1000) { echo "<option value='".$homo."'>".$homo."</option>".$homo++; } 
	}
