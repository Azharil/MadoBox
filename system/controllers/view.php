<?php
	...
	if( $this->param('post') ) {
		...
		$group_id   = intval($this->param('post'));
	}
	...
	if( $post_type == 'public' && $D->post->post_user->id>0 ) {
		$tmp = $db2->fetch_field('SELECT id FROM posts WHERE id>"'.$D->post->post_id.'" AND group_id="'.$D->post->post_group->id.'" AND user_id="'.$D->post->post_user->id.'" ORDER BY id ASC LIMIT 1');
		if($tmp) {
			$D->prevpost	= $C->SITE_URL.'view/'.$D->post->post_group->groupname.'/post:'.$tmp;
		}
		$tmp = $db2->fetch_field('SELECT id FROM posts WHERE id<"'.$D->post->post_id.'" AND group_id="'.$D->post->post_group->id.'" AND user_id="'.$D->post->post_user->id.'" ORDER BY id DESC LIMIT 1');
		if($tmp) {
			$D->nextpost	= $C->SITE_URL.'view/'.$D->post->post_group->groupname.'/post:'.$tmp;
		}
		unset($tmp);
	}
