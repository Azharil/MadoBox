	...
	if($D->tab == 'updates') {
		...
		if( !$D->posts_protected ){
			...
			if($D->filter == 'all') {
				...
				$q2	= 'SELECT p.*, p.id AS pid, "public" AS `type` FROM posts p WHERE p.user_id="'.$u->id.'" AND p.group_id<>2000  AND p.api_id<>2 AND p.api_id<>6 '.$start_from.' ORDER BY p.id DESC ';
